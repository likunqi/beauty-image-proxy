        ...
        if (route === "upload-random") {
          ...
          while (results.length < count && budget-- > 0) {
            ...
            try {
              ...
              let imgResp;
              try { imgResp = await fetch(imgResult.url, { signal: dlCtl.signal }); } finally { clearTimeout(dlTimer); }
              ...
              let uploadResp;
              try { uploadResp = await fetch("https://likunqi.top/upload", { method: "POST", body: formData, signal: upCtl.signal }); } finally { clearTimeout(upTimer); }
              if (!uploadResp.ok) throw new Error("upload HTTP " + uploadResp.status);
              const uploadText = await uploadResp.text();
              let uploadData;
              try { uploadData = JSON.parse(uploadText); } catch { throw new Error("upload non-JSON: " + uploadText.slice(0, 100)); }
...
<code>GET /api/v1/image/:name</code> — 指定源 → <a href="/api/v1/image/xxapi">/api/v1/image/xxapi</a><br>

<code>GET /api/v1/image/:name?redirect</code> — 指定源 302 跳转<br>

<code>GET /api/v1/upload-random?count=N</code> 上传 N 张图片到图床<br>
